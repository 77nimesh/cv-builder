...
export default function MinimalBandTemplateNine({
  data,
  photoPath = null,
  editable = false,
  ...
  const contentSections = visibleSections.filter(
    (section) => section.type !== "personal-details"
  );
  ...
  function renderTopBand() {
    ...
    return (
      <header
        className="print-avoid-break px-10 py-5"
        style={{ backgroundColor: theme.primary, color: theme.onPrimary }}
      >
        {hasText(personal.fullName) ? (
          <h1 className="break-words text-3xl font-semibold leading-tight tracking-[-0.03em]">
            {personal.fullName}
          </h1>
        ) : null}
  ...
  function renderIdentityStrip() {
    if (!showPersonal && !hasText(normalizedPhotoPath)) {
      return null;
    }

    return (
      <div className="print-avoid-break flex items-center justify-between gap-6 px-10 py-6">
        <div className="min-w-0 flex-1">
          {hasText(contactLine) ? (
            <p className="break-words text-sm leading-6 text-slate-600">{contactLine}</p>
          ) : null}
        </div>
