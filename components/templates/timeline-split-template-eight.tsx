...
export default function TimelineSplitTemplateEight({
  data,
  photoPath = null,
  editable = false,
  ...
  function renderPhoto() {
    ...
    return (
      <ResumePhoto
        ...
        shape={photoShape}
        squareClassName="rounded-[1.75rem]"
    ...
    return (
      <div className={sidebar ? "mb-3" : "mb-5"}>
        ...
        <div
          ...
          style={{ backgroundColor: sidebar ? theme.softBorder : theme.primary }}
        />
      </div>
    );
  }
  ...
  function renderSectionShell(
    ...
    const sidebar = zone === "sidebar";

    return (
      <section
        key={section.id}
        className={`resume-section-block ${
          sidebar ? "rounded-3xl bg-white/70 p-4 shadow-sm" : "rounded-[1.75rem] bg-white p-6 shadow-sm"
      ...
      >
        ...
        {renderSectionHeading(section, sidebar)}
        {content}
      </section>
    );
  }

  function renderTimelineExperienceSection(section: ResumeSection) {
    const orderedItems = getOrderedItems(section);
    const itemDragEnabled = editable && supportsItemDrag(section);
        ...
        {orderedItems.length > 0 ? (
          orderedItems.map((item, index) => {
            ...
            return renderItemShell(
              section,
              item,
              <article className="relative grid grid-cols-[26px_1fr] gap-4">
                <div className="relative flex justify-center">
                  {!isLastItem ? (
                    <span
                      className="absolute top-3 h-[calc(100%+1.25rem)] w-px"
                      style={{ backgroundColor: theme.softBorder }}
                    />
                  ) : null}
                  <span
                    className="relative mt-1 h-4 w-4 rounded-full border-4 border-white shadow-sm"
        ...
        {skills.length > 0 ? (
          skills.map(({ item, skill }) =>
            renderItemShell(
              section,
              item,
              <div className="flex items-start justify-between gap-3 rounded-2xl bg-white/70 px-3 py-2">
    ...
    switch (section.type) {
      case "experience":
        return renderSectionShell(
          section,
          zone,
          zone === "main" ? renderTimelineExperienceSection(section) : renderExperienceSection(section)
        );
      case "education":
  ...
  return (
    <article
      className="resume-preview-grid grid min-h-[1123px] grid-cols-[250px_1fr] overflow-hidden bg-white text-slate-900"
    ...
    >
          ...
          {showPersonal ? (
            <section className="print-avoid-break">
              {hasText(personal.fullName) ? (
                <h1 className="break-words text-3xl font-black leading-none tracking-[-0.05em] text-slate-950">
                  {personal.fullName}
                </h1>
              ) : null}
              {hasText(personal.headline) ? (
                <p
                  className="mt-3 text-xs font-black uppercase leading-5 tracking-[0.18em]"
      ...
      >
        <div className="space-y-5">
          {showSummary ? (
            <section className="rounded-[1.75rem] bg-white p-6 shadow-sm print-avoid-break">
              <p
